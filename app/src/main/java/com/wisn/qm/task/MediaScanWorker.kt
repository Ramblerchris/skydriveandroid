    ...
    override suspend fun doWork(): Result {
        try {
            ...
            if (maxId != null && maxId > 0) {
                ...
                // todo 再次扫描更新
                val mediaImageListNew =
                    DataRepository.getInstance().getMediaImageAndVideoList(maxId.toString())
                var start11 = System.currentTimeMillis();
                LogUtils.d(TAG, " 查询最大值maxId列表用时： " + (start11 - start))

                var mediaInfoListAll =
                    AppDataBase.getInstanse().mediaInfoDao?.getMediaInfoListAllNotDelete()
