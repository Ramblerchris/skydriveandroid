...
import com.library.base.utils.IOUtils;
import com.library.base.utils.ThreadPoolManager;
...
public class HttpLoggingInterceptor implements Interceptor {
    ...
    public void logToLongString(String log, ArrayList<String> logArray) {
        ...
        if (length > maxLenght) {
            int start = 0;
            int end = maxLenght;
            for (int i=0; i < (length / maxLenght+1); i++) {
                if (length > end) {
                    if (i == 0) {
                        logArray.add("\tbody:" + log.substring(start, end));
                    } else {
                        logArray.add(log.substring(start, end));
                    }
                    start = end;
                    end = end + maxLenght;
                } else {
                    logArray.add(log.substring(start, length));
                    break;
                }
            }
        } else {
