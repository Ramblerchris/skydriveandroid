...
pwd
mkdir ${BuildLog}/
touch ${BuildLog}/newReleasebuildlog.txt
sh ./gradlew clean assembleRelease | grep -v 'newDebugbuildlog.txt' > ${BuildLog}/newReleasebuildlog.txt
...
echo 'BUILDFAILED'
cat ${BuildLog}/newReleasebuildlog.txt
exit
fi
done <${BuildLog}/newReleasebuildlog.txt
...
echo 开始4k对齐
ls ${WORKSPACE}/build/outputs/apk/release/ |grep -v 'apkduiqilist.txt' > ${WORKSPACE}/build/apkduiqilist.txt

duiqiisOk=false
while read lineduiqi
do
  if [[ "${lineduiqi##*.}"x = "apk"x ]];then
    echo ${WORKSPACE}/build/outputs/apk/release/${lineduiqi}
    ${buildtools25}/zipalign -v 4   ${WORKSPACE}/build/outputs/apk/release/${lineduiqi}  ${WORKSPACE}/build/outputs/duiqi/${lineduiqi}
        duiqiisOk=ok
  fi
